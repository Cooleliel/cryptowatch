...
void main() {
  group('fetchTopCryptos - cas nominal', () {
    test('retourne les cryptos de CoinGecko', () async {
      final client = _FakeHttpClient(
        ...
        body: jsonEncode(<Map<String, dynamic>>[
          _validEntry(id: 'bitcoin', name: 'Bitcoin', symbol: 'btc', price: 45000.0),
          _validEntry(id: 'ethereum', name: 'Ethereum', symbol: 'eth', price: 3000.0),
        ]),
      );
    ...
    test('retourne une liste vide si CoinGecko renvoie une liste vide', () async {
      final client = _FakeHttpClient(statusCode: 200, body: jsonEncode(<Object>[]));
  ...
  group('fetchTopCryptos - donnees invalides', () {
    test("leve InvalidDataException si la reponse n'est pas une liste", () async {
      final client = _FakeHttpClient(
        statusCode: 200,
    ...
    test("leve InvalidDataException si le corps n'est pas du JSON", () async {
      final client = _FakeHttpClient(
        statusCode: 200,
    ...
    test('ignore un item invalide et retourne les autres', () async {
      final client = _FakeHttpClient(
        ...
        body: jsonEncode(<Map<String, dynamic>>[
          ...
          <String, dynamic>{
            ...
            'name': 'Cassee',
            'symbol': 'brk',
            'current_price': null, // champ requis invalide
          },
          _validEntry(id: 'ethereum', name: 'Ethereum', symbol: 'eth', price: 3000.0),
        ]),
      );
      ...
      expect(cryptos, hasLength(2));
      expect(
        cryptos.map((crypto) => crypto.id),
        <String>['bitcoin', 'ethereum'],
      );
    });

    test("ignore un item qui n'est pas un objet JSON", () async {
      final client = _FakeHttpClient(
        statusCode: 200,
        body: jsonEncode(<Object>[
          _validEntry(id: 'bitcoin', name: 'Bitcoin', symbol: 'btc', price: 45000.0),
  ...
  group('contrat du repository', () {
    test('toutes les erreurs sortent en MarketException, jamais autre chose', () async {
      final clients = <_FakeHttpClient>[
        ...
        _FakeHttpClient(statusCode: 200, body: jsonEncode(<String, int>{'a': 1})),
      ];
